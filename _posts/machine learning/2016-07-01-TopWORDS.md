...
因为$\vec{\theta}$隐含了$D$，为了简化书写，我在下文推导中省略了$D$。推导中采用了Word Dictionary Model(WDM)模型，即$P(S|\vec{\theta})=\prod\limits{k=1}^K\theta_{ik}$。另外，$S$和$T$的关系如下：$P(T|\vec{\theta})=\sum\limits_{S\in C_T}P(S|\vec{\theta})$, 其中$C_T$表示$T$所有可能的分词结构。
